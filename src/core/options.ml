...
let opt_unsound_recursion = ref false
...
let opts =
  ...
  ("-pfs_formula", (Arg.String (fun s -> opt_pfs_formula := s)), "Partial flow-sensitivity based on boolean formula (require a formula json file)");
  ...
  ("-unsound_recursion", (Arg.Set opt_unsound_recursion), "Unsound recursive calls");
